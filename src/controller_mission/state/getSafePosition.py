...
class GetSafePosition(MissionState):
    ...
    def request_pose(self):
        rate = rospy.Rate(1)
        msg = PoseSavedRequest()
        msg.id = uint8(self.param_position_id)
        while self.pose_is_not_get:
            self.pub_request_position.publish(msg)
            rate.sleep()

    def get_pose_cb(self, data):
        self.pose = data

    def initialize(self):
        self.pub_request_position = rospy.Publisher('/proc_mapping/GetSavedPoseRequest',
                                                    PoseSavedRequest, queue_size=100)

        self.sub_get_pose = rospy.Subscriber('/proc_mapping/GetSavedPoseResponse',
                                             KeyValueIdPose, self.get_pose_cb)
