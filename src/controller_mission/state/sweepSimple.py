...
class SweepSimple(MissionState):
    ...
    def run(self, ud):
        if self.target_reached:
            if self.one_more_target_reached == 1:
                self.position = self.referencial_change(0.0, self.param_distance_y, self.position_x, self.position_y, 90.0)
                self.set_target(self.position[0], self.position[1], self.base_heading)

            elif self.one_more_target_reached == 2:
                self.position = self.referencial_change(0.0, self.param_distance_y * 2,
                                                        self.position[0], self.position[1], -90.0)
                self.set_target(self.position[0], self.position[1], self.base_heading)

            elif self.one_more_target_reached == 3:
                self.set_target(self.position_x, self.position_y, self.base_heading)
